...
# Provider configs — all via OpenAI-compatible APIs (no paid OpenAI required)
_PROVIDERS = {
    "gemini": {
...
def _build_client():
    ...
    if cfg is None:
        raise ValueError(
            f"Unknown MODEL_PROVIDER '{provider}'. Choose: {', '.join(_PROVIDERS)}"
        )

    api_key = next(
        (os.getenv(k) for k in cfg["env_key"] if os.getenv(k)), None
    )
    if not api_key:
        keys = " or ".join(cfg["env_key"])
        raise ValueError(
            f"{keys} is not set. "
            + (
                "Get a free Gemini key at https://aistudio.google.com/"
                if provider == "gemini"
                else "Get a DeepSeek key at https://platform.deepseek.com/"
            )
        )
